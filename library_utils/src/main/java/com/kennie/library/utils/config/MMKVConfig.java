 ...
 * @创建人 Kennie
 * @修改人
 * @创建时间 2021/11/18 1:07
 */
public class MMKVConfig implements IMMKVConfig {

    private static MMKVConfig mmkvConfig;
    ...
    public static MMKVConfig getMMKVConfig() {
        return mmkvConfig;
    }

    public MMKVConfig(Context mContext) {
        this.mContext = mContext;
        loadConfig();
    }

    public MMKVConfig(Context mContext, String mmkvName) {
        this.mContext = mContext;
    ...
    public static MMKVConfig init(Context context) {
        if (mmkvConfig == null) {
            synchronized (MMKVConfig.class) {
                if (mmkvConfig == null) {
                    mmkvConfig = new MMKVConfig(context);
                }
            }
        }
        return mmkvConfig;
    ...
    public static MMKVConfig init(Context context, String mmkvName) {
        if (mmkvConfig == null) {
            synchronized (MMKVConfig.class) {
                if (mmkvConfig == null) {
                    mmkvConfig = new MMKVConfig(context, mmkvName);
                }
            }
        }
        return mmkvConfig;
    ...
    public void loadConfig() {
        String rootDir = MMKV.initialize(mContext);//初始化mmkv
        System.out.println("mmkv root: " + rootDir);
        mmkv = MMKV.mmkvWithID("app_config", MMKV.MULTI_PROCESS_MODE);
    }
